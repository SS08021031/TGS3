...
{
    public static Slime02 instance;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("line"))
        {
            if (isBind == false)
            {
                ...
                Audio_Manager.instance.PlaySE(6);
                Destroy(this.gameObject);
                var child = Instantiate(SlimeImpact, transform.position, Quaternion.identity);
                child.transform.parent = parentObject.transform;

                //スコアが上がる
                score_test.instance.score += 100;
